...
const MOUNT_PATH =
  process.env.MOUNT_PATH ?? path.join(process.cwd(), "problems");
...
async function getProblemFullBoilerplateCode(
  ...
  return new Promise((resolve, reject) => {
    fs.readFile(
      path.join(
        MOUNT_PATH,
        `/${problemId}/boilerplate-full/function.${languageId}`
      ),
      { encoding: "utf-8" },
      (err, data) => {
        if (err) {
...
async function getProblemInputs(problemId: string): Promise<string[]> {
  return new Promise((resolve, reject) => {
    fs.readdir(
      path.join(MOUNT_PATH, `/${problemId}/tests/inputs`),
      async (err, files) => {
        if (err) {
          console.log(err);
          reject(err);
          ...
          await Promise.all(
            files.map((file) => {
              return new Promise<string>((resolve, reject) => {
                fs.readFile(
                  path.join(MOUNT_PATH, `/${problemId}/tests/inputs/${file}`),
                  { encoding: "utf-8" },
                  (err, data) => {
                    if (err) {
                      reject(err);
...
async function getProblemOutputs(problemId: string): Promise<string[]> {
  return new Promise((resolve, reject) => {
    fs.readdir(
      path.join(MOUNT_PATH, `/${problemId}/tests/outputs`),
      async (err, files) => {
        if (err) {
          console.log(err);
          reject(err);
          ...
          await Promise.all(
            files.map((file) => {
              return new Promise<string>((resolve, reject) => {
                fs.readFile(
                  path.join(MOUNT_PATH, `/${problemId}/tests/outputs/${file}`),
                  { encoding: "utf-8" },
                  (err, data) => {
                    if (err) {
                      reject(err);
